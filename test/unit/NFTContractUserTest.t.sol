...
contract TestUserFunctions is Test {
    ...
    function setUp() external virtual {
        ...
        token = ERC20Token(nftContract.getFeeToken());
    }
    ...
    function fund(address account) public {
        ...
        address owner = token.owner();
        vm.prank(owner);
        token.transfer(account, 500_000_000 ether);
    }
     ...
     * TRAIT ASSIGNMENT
     */
    function test__unit__AssignsCorrectTrait(uint256 randNum) public funded(USER) unpaused skipFork {
        vm.prevrandao(bytes32(randNum));
        ...
        bytes memory tokenUri = bytes(nftContract.tokenURI(1));
        uint256 tokenUriNumber;
        bytes memory delimiter = bytes("/");
        for (uint256 i = tokenUri.length - 1; i >= 0; i--) {
            bytes memory character = new bytes(1);
            character[0] = tokenUri[i];

            if (keccak256(abi.encodePacked(delimiter)) == keccak256(abi.encodePacked(character))) {
                uint256 numLength = tokenUri.length - i - 1;
                // console.log(numLength);
                bytes memory substring = new bytes(numLength);
                for (uint256 j = 0; j < numLength; j++) {
                    substring[j] = tokenUri[i + 1 + j];
                }
                // console.log(string(substring));
                tokenUriNumber = StringToNumber.stringToUint(string(substring));
                break;
            }
        }
